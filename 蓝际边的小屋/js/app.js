...
function renderWorks(works) {
    ...
    grid.innerHTML = works.map(function (w) {
        var coverContent = '';
        if (w.cover_url) {
            coverContent = '<img src="' + esc(w.cover_url) + '" alt="' + esc(w.title) + '" style="width:100%;height:100%;object-fit:cover">';
        } else {
            coverContent = esc(w.cover || '📖');
        }
        return '<article class="work-card" onclick="window.location.href=\'pages/detail.html?id=' + w.id + '\'">' +
            '<div class="work-cover" style="--cover-bg:' + esc(w.cover_bg || 'linear-gradient(135deg,#94C2E8,#345891)') + '">' + coverContent + '</div>' +
            '<div class="work-body">' +
...
async function renderDetailPage() {
    ...
    var coverHtml = '';
    if (work.cover_url) {
        coverHtml = '<img src="' + esc(work.cover_url) + '" alt="' + esc(work.title) + '" style="width:100%;height:100%;object-fit:cover;border-radius:var(--radius-lg)">';
    } else {
        coverHtml = esc(work.cover || '📖');
    }
    var downloadHtml = '';
    if (work.file_url) {
        var fileLabel = work.file_type === 'txt' ? '📥 下载 TXT' : '📥 下载 Epub';
        downloadHtml = '<div style="margin-top:14px"><a href="' + esc(work.file_url) + '" class="btn btn-sm btn-download" download>' + fileLabel + '</a></div>';
    }
    c.innerHTML =
        '<div class="detail-header">' +
            '<div class="detail-cover" style="background:' + esc(work.cover_bg || 'linear-gradient(135deg,#94C2E8,#345891)') + '">' + coverHtml + '</div>' +
...
function parseTxtChapters(text) {
    // 匹配 "第X章"、"Chapter X"、"第X话" 等常见格式
    var patterns = [
        /第[零一二三四五六七八九十百千万\d]+[章节话回].*?\n/g,
        /Chapter\s+\d+.*?\n/gi,
        /第[零一二三四五六七八九十百千万\d]+[章节话回].*/g,
        /Chapter\s+\d+.*/gi
    ];
    var matches = [];
    for (var p = 0; p < patterns.length; p++) {
        matches = text.match(patterns[p]);
    ...
    for (var i = 0; i < matches.length; i++) {
        var title = matches[i].replace(/[\r\n]/g, '').trim() || ('第' + (i + 1) + '章');
        var start = text.indexOf(matches[i]) + matches[i].length;
        var end = i + 1 < matches.length ? text.indexOf(matches[i + 1]) : text.length;
        chapters.push({ title: title, content: text.slice(start, end).trim() });
    }
    return chapters;
}
window.parseTxtChapters = parseTxtChapters;
...
async function seedData() {
    ...
    var works = [
        { id: 1, type: 'novel', title: '夜行猫与流星伞', author: '柊 千夜', cover: '🐱', cover_url: '', tags: ['治愈', '奇幻', '日常'], description: '一只会说话的黑猫，带着捡来的伞，在下雨的城市里寻找回家的路。', cover_bg: 'linear-gradient(135deg,#D6E8F8,#345891)', rating: 4.8, updated: '更新至 3 话', file_url: '', file_type: '' },
        { id: 2, type: 'novel', title: '边境的钟表匠', author: '水無月 蒼', cover: '⏰', cover_url: '', tags: ['奇幻', '冒险', '温情'], description: '在被时间遗忘的小镇上，钟表匠与迷路的旅人共同修复一座停摆的巨钟。', cover_bg: 'linear-gradient(135deg,#7298E6,#203A60)', rating: 4.9, updated: '连载中', file_url: '', file_type: '' },
        { id: 3, type: 'manga', title: '纸伞下的怪谈铺', author: '桔梗 灯里', cover: '👻', cover_url: '', tags: ['怪谈', '悬疑', '治愈'], description: '专门收购奇怪故事的小铺子，每一个来客都带着一段不可思议的过往。', cover_bg: 'linear-gradient(135deg,#94C2E8,#345891)', rating: 4.6, updated: '更新至 18 卷', file_url: '', file_type: '' },
        { id: 4, type: 'anime', title: '星海图书馆的看守人', author: '天野 灯', cover: '🌟', cover_url: '', tags: ['科幻', '治愈', '文艺'], description: '在漂浮于宇宙中的图书馆里，看守人负责为每一本孤独的书找到读者。', cover_bg: 'linear-gradient(135deg,#E0EDFA,#6398D1)', rating: 4.7, updated: '已完结', episodes: 24, file_url: '', file_type: '' },
        { id: 5, type: 'anime', title: '放学后的结界部', author: '百目鬼 澪', cover: '🎭', cover_url: '', tags: ['校园', '奇幻', '热血'], description: '三个平凡的高中生意外接手了守护校园结界的任务。', cover_bg: 'linear-gradient(135deg,#1A30B8,#7298E6)', rating: 4.5, updated: '连载中', file_url: '', file_type: '' },
        { id: 6, type: 'manga', title: '旧书堂的猫店长', author: '小鹿 白', cover: '📚', cover_url: '', tags: ['日常', '萌系', '治愈'], description: '一间快要倒闭的旧书店，因为一只神秘的猫而重新热闹起来。', cover_bg: 'linear-gradient(135deg,#D8E4F7,#203A60)', rating: 4.9, updated: '更新至 9 卷', file_url: '', file_type: '' }
    ];
    var chapters = [
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (sidebar) {
        (async function () {
            ...
            if (addWorkForm) addWorkForm.addEventListener('submit', async function (e) {
                ...
                var btn = document.getElementById('addWorkBtn');
                ...
                if (!type || !title || !author || !tags.length || !desc) { showToast('请填写完整信息', 'warning'); btn.disabled = false; return; }

                var statusEl = document.getElementById('uploadStatus');
                statusEl.style.display = 'inline';
                statusEl.textContent = '⏳ 上传中...';

                var coverFile = document.getElementById('coverFileInput').files[0];
                var novelFile = document.getElementById('fileInput').files[0];
                var cover_url = '';
                var file_url = '';
                var file_type = '';
                ...
                if (coverFile) {
                    try {
                        var coverPath = 'covers/' + Date.now() + '_' + coverFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
                        var upResult = await db.storage.from('covers').upload(coverPath, coverFile, { cacheControl: '3600', upsert: true });
                        if (upResult.error) throw upResult.error;
                        var urlResult = db.storage.from('covers').getPublicUrl(coverPath);
                        cover_url = urlResult.data.publicUrl;
                    } catch (err) {
                        btn.disabled = false;
                        statusEl.style.display = 'none';
                        showToast('封面上传失败: ' + (err.message || '未知错误'), 'error');
                        return;
                    }
                }
                ...
                if (novelFile) {
                    try {
                        var filePath = 'novels/' + Date.now() + '_' + novelFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
                        var fResult = await db.storage.from('novels').upload(filePath, novelFile, { cacheControl: '3600', upsert: true });
                        if (fResult.error) throw fResult.error;
                        var furlResult = db.storage.from('novels').getPublicUrl(filePath);
                        file_url = furlResult.data.publicUrl;
                        file_type = novelFile.name.toLowerCase().endsWith('.epub') ? 'epub' : 'txt';
                    } catch (err) {
                        btn.disabled = false;
                        statusEl.style.display = 'none';
                ...
                var workData = {
                    ...
                    tags: tags,
                    description: desc,
                    cover_bg: 'linear-gradient(135deg,#94C2E8,#345891)',
                    rating: 0,
                    updated: '新发布',
                    file_url: file_url,
                    file_type: file_type
                ...
                var result = await db.from('works').insert([workData]);
                if (result.error) {
                    btn.disabled = false;
                    statusEl.style.display = 'none';
                ...
                // TXT 自动分章
                if (file_type === 'txt' && novelFile) {
                    try {
                        var txtContent = await novelFile.text();
                        var chaps = parseTxtChapters(txtContent);
                        if (chaps.length > 0) {
                            var chapData = chaps.map(function (ch, i) {
                                return { id: Date.now() + i + 1, work_id: workId, title: ch.title, content: ch.content, sort_order: i + 1 };
                            });
                            await db.from('chapters').insert(chapData);
                        }
                    } catch (err) {
                        console.warn('TXT 分章失败:', err.message);
                    }
                }
                ...
                updateChapterWorkSelect(); renderAdminWorks();
            });
            ...
            if (workTypeSelect) {
                workTypeSelect.addEventListener('change', function () {
                    var group = document.getElementById('fileUploadGroup');
                    if (this.value === 'novel') {
    ...
    console.log(db ? '✅ 蓝际边的小屋已就绪 (Supabase 模式)' : '⚠️ Supabase 未连接');
});
